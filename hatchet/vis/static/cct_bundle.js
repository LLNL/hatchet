    ...
    value: function _buildDummyHolder(protoype, parent, elided) {
      ...
      try {
        for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
          ...
          var aggMetsForChild = this._getAggregateMetrics(_elided, globals.AVG);
...
var ChartView = /*#__PURE__*/function (_View2) {
  ...
  cct_chart_view_createClass(ChartView, [{
    ...
    value: function manageLabelCollisions(nodes, entrflg) {
      var _this5 = this;
    ...
    value: function _preRender() {
      ...
      var mainG = this.svg.append("g").attr('id', "mainG").attr("transform", "translate(" + globals.layout.margin.left + "," + globals.layout.margin.top + ")"); // .nodeSize([this._maxNodeRadius, this._maxNodeRadius]);
...
var ScentedSliderPopup = /*#__PURE__*/function (_View) {
  ...
  function ScentedSliderPopup(elem, model) {
    ...
    _this.full_hist_height = _this.popup_dims.height * .6;
    _this.num_bins = 25;
    _this.slider_width = 10;
    _this.slider_height = 20;
    _this._svg = build_d3.select(elem).append('svg').attr('width', _this.popup_dims['width']).attr('height', _this.popup_dims['height']).style('position', 'absolute') // .style('left', (this.elem.clientWidth*.5 - this.popup_dims.width*.5) + 'px')
  ...
  cct_scented_slider_popup_createClass(ScentedSliderPopup, [{
    ...
    value: function manage_slider_update(slider, x_pos) {
      ...
      if (update) {
        self.update = true;
        slider.attr('transform', "translate(".concat(step_loc - self.slider_width / 2, ",0)")); // slider.select('text').text(`${range_val}`);
    ...
    value: function render() {
      ...
      bars.enter().append('rect').attr('class', 'hist-bar').attr('width', (this.popup_dims['width'] - this.popup_dims.left_padding - this.popup_dims.right_padding) / this.bins.length).attr('height', function (d) {
        if (d.length != 0) {
          return _this3.h_y_scale(d.length);
        }
        ...
        if (d.length != 0) {
          return _this3.hist_height - _this3.h_y_scale(d.length); //- v_bar_negative_padding;
        }

        return _this3.hist_height;
      }).attr('fill', function (_, i) {
        if (i >= _this3.current_l_bin && i <= _this3.current_r_bin) {
          return 'rgba(100,100,200,1)';
        }
      }).attr('stroke', 'rgba(0,0,0,1)').attr('stroke-width', '1px').on('mouseenter', function (d) {
        _this3.observers.notify({
      ...
      rev_bars.enter().append('rect').attr('class', 'ice-bar').attr('width', (this.popup_dims['width'] - this.popup_dims.left_padding - this.popup_dims.right_padding) / this.bins.length).attr('height', function (d) {
        if (d.length != 0) {
          return _this3.i_y_scale(d.length);
        }
