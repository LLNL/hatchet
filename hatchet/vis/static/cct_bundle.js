...
var build_d3 = __webpack_require__(93);
;// CONCATENATED MODULE: ./scripts/cct/cct_globals.js
...
var digitAbbrevScale = build_d3.scaleOrdinal().range(["K", "K", "K", "M", "M", "M", "B", "B", "B", "T", "T", "T"]).domain(new Array(12).fill(0).map(function (_, i) {
  return i + 4;
}));
...
function getSigFigString(num) {
  ...
  if (num.toFixed(2).length <= 6) {
    ...
    for (var i = 4; i <= numdig; i += 3) {
      num = parseInt(num) / 1000;
    }

    var numstr = num.toFixed(2).toString();
    var abbrev = digitAbbrevScale(numdig);
      ...
      return function (evt) {
        ...
        switch (evt.type) {
          case globals.signals.UPDATESELECTED:
            _this.model.updateSelected(evt.nodes);

            break;

          case globals.signals.CLICK:
            _this.model.updateSelected(evt.node);
          ...
          case globals.signals.TOGGLEBRUSH:
            _this.model.toggleBrush();

            break;

          case globals.signals.BRUSH:
            _this.model.setBrushedPoints(evt.selection);

            break;

          case globals.signals.METRICCHANGE:
            _this.model.changeMetric(evt.newMetric, evt.source);

            break;

          case globals.signals.COLORCLICK:
            _this.model.changeColorScheme(evt.value);

            break;

          case globals.signals.TREECHANGE:
            _this.model.updateActiveTrees(evt.display);

            break;

          case globals.signals.LEGENDCLICK:
            _this.model.updateLegends(evt.value);

            break;

          case globals.signals.ENABLEMASSPRUNE:
            _this.model.enablePruneTree(evt.threshold);
...
/* harmony default export */ const cct_controller = (Controller);
...
;// CONCATENATED MODULE: ./scripts/cct/cct_stats.js
function cct_stats_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var Stats = /*#__PURE__*/function () {
  function Stats() {
    cct_stats_classCallCheck(this, Stats);
  }

  cct_stats_createClass(Stats, [{
    key: "_asc",
    value: function _asc(arr, metric) {
      /**
    ...
    key: "_quantile",
    value: function _quantile(arr, q, metric) {
      /**
       ...
       */
      var sorted = this._asc(arr, metric);
    ...
    key: "_getIQR",
    value: function _getIQR(arr, metric) {
      /**
...
/* harmony default export */ const cct_stats = (Stats);
...
function cct_repr_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var Forest = /*#__PURE__*/function () {
  function Forest(cct_forest_def) {
    cct_repr_classCallCheck(this, Forest);

    //initialize forest containers
    this.immutableTrees = [];
    this.prePrunedTrees = [];
    this.mutableTrees = []; //initialize forest descriptors

    this.numberOfTrees = cct_forest_def.length;
    this.metricColumns = build_d3.keys(cct_forest_def[0].metrics);
    this.attributeColumns = build_d3.keys(cct_forest_def[0].attributes);
    this.aggregateMinMax = {};
    this.maxHeight = 0;
    this.forestMinMax = {};
    this.forestMetrics = [];
    this.rootNodeNames = [];
    this.stats = new cct_stats();
    this.zeroes = false;
    this.subtreeMap = {}; //setup functions

    this.instantiateTrees(cct_forest_def);
    this.organizeMetrics(cct_forest_def);
  }

  cct_repr_createClass(Forest, [{
    key: "hashNode",
    value: function hashNode(n) {
      return n._hatchet_nid;
    }
  }, {
    key: "postOrderCheck",
    value: function postOrderCheck(t) {
      /**
      ...
      if (t.children !== undefined) {
        var _iterator = _createForOfIteratorHelper(t.children),
            _step;
    ...
    key: "findDuplicateSubtrees",
    value: function findDuplicateSubtrees() {
      /**
       * Wrapper function to idenfity duplicate subtrees.
       */
      var _iterator2 = _createForOfIteratorHelper(this.immutableTrees),
          _step2;
    ...
    key: "instantiateTrees",
    value: function instantiateTrees(forestData) {
      /**
      ...
      for (var treeIndex = 0; treeIndex < forestData.length; treeIndex++) {
        var hierarchy = hierarchy_hierarchy(forestData[treeIndex], function (d) {
          return d.children;
        });
        hierarchy.size = hierarchy.descendants().length; //add a local nid to this representation
      ...
      this.immutableTrees.sort(function (a, b) {
        return b.size - a.size;
      });
      this.mutableTrees.sort(function (a, b) {
        return b.size - a.size;
      });
    }
  }, {
    key: "initializePrunedTrees",
    value: function initializePrunedTrees(primaryMetric) {
      /**
       ...
       */
      var newTrees = this.getFreshTrees();

      for (var t in newTrees) {
        var pt = this.pruneTree(newTrees[t], primaryMetric, function (h, primaryMetric) {
      ...
      this.prePrunedTrees.sort(function (a, b) {
        return b.size - a.size;
      });
      this.mutableTrees.sort(function (a, b) {
        return b.size - a.size;
    ...
    key: "organizeMetrics",
    value: function organizeMetrics(forestData) {
      var _this = this;
      ...
      var _loop = function _loop() {
        thisTree = forestData[index];
        var mc = _this.metricColumns; // Get tree names for the display select options

        _this.rootNodeNames.push(thisTree.frame.name);
        ...
        for (j = 0; j < mc.length; j++) {
          ...
          thisTreeMetrics[mc[j]]["max"] = Number.MIN_SAFE_INTEGER; //only one run time
        ...
        _this.immutableTrees[index].each(function (d) {
      ...
      this.forestMetrics = _forestMetrics; // Global min/max are the last entry of forestMetrics;
    ...
    key: "_setPruneFlags",
    value: function _setPruneFlags(tree, primaryMetric, callback) {
      /**
    ...
    key: "_getAggregateMetrics",
    value: function _getAggregateMetrics(h, aggFunct) {
      /**
      ...
      var _iterator3 = _createForOfIteratorHelper(this.metricColumns),
          _step3;
      ...
      try {
        var _loop2 = function _loop2() {
          ...
          switch (aggFunct) {
            ...
            default:
              console.warn("".concat(aggFunct, " is not supported for aggregating subtrees."));
          }
        };
    ...
    key: "_getSubTreeDescription",
    value: function _getSubTreeDescription(h) {
      /**
    ...
    key: "_buildDummyHolder",
    value: function _buildDummyHolder(protoype, parent, elided) {
      /**
      ...
      dummyHolder.outlier = 0; //initialize the aggregrate metrics for summing

      var _iterator4 = _createForOfIteratorHelper(this.metricColumns),
          _step4;
      ...
      var _iterator5 = _createForOfIteratorHelper(dummyHolder.data.elided),
          _step5;

      try {
        for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
          var _elided = _step5.value;

          var aggMetsForChild = this._getAggregateMetrics(_elided, globals.SUM);

          var descriptionOfChild = this._getSubTreeDescription(_elided);

          var _iterator8 = _createForOfIteratorHelper(this.metricColumns),
              _step8;

          try {
            for (_iterator8.s(); !(_step8 = _iterator8.n()).done;) {
              var _metric = _step8.value;
              aggregateMetrics[_metric] += aggMetsForChild[_metric];
            }
          } catch (err) {
            _iterator8.e(err);
          } finally {
      ...
      var _iterator6 = _createForOfIteratorHelper(this.metricColumns),
          _step6;

      try {
        for (_iterator6.s(); !(_step6 = _iterator6.n()).done;) {
          var _metric2 = _step6.value;
          aggregateMetrics[_metric2] = aggregateMetrics[_metric2] / elided.length;
        }
      } catch (err) {
        _iterator6.e(err);
      } finally {
      ...
      description.elidedSubtrees = elided.length; //get the overall min and max of aggregate metrics
      // for scales

      var _iterator7 = _createForOfIteratorHelper(this.metricColumns),
          _step7;

      try {
        for (_iterator7.s(); !(_step7 = _iterator7.n()).done;) {
          var _metric3 = _step7.value;

          if (aggregateMetrics[_metric3] > this.aggregateMinMax[_metric3].max) {
            this.aggregateMinMax[_metric3].max = aggregateMetrics[_metric3];
          }

          if (aggregateMetrics[_metric3] < this.aggregateMinMax[_metric3].min) {
            this.aggregateMinMax[_metric3].min = aggregateMetrics[_metric3];
          }
        }
      } catch (err) {
        _iterator7.e(err);
    ...
    key: "_pruningVisitor",
    value: function _pruningVisitor(root, condition, metric) {
      /**
      ...
      if (root.children) {
        ...
        for (var childNdx = root.children.length - 1; childNdx >= 0; childNdx--) {
          var child = root.children[childNdx]; //clear dummy node codition so it
          // doesnt carry on between re-draws

          child.aggregate = false; //condition where we remove child
        ...
        try {
          for (_iterator9.s(); !(_step9 = _iterator9.n()).done;) {
            ...
            this._pruningVisitor(_child, condition, metric);
          }
        } catch (err) {
          _iterator9.e(err);
        } finally {
    ...
    key: "_setDisplayFlags",
    value: function _setDisplayFlags(t, primaryMetric, conditionCallback) {
      /**
    ...
    key: "pruneTree",
    value: function pruneTree(t, primaryMetric, conditionCallback) {
      /**
       ...
       */
      this._setDisplayFlags(t, primaryMetric, conditionCallback); //The sum ensures that we do not prune 
      ...
      this._pruningVisitor(t, 1, primaryMetric); //update size of subtrees on the nodes
    ...
    key: "aggregateTreeData",
    value: function aggregateTreeData(primaryMetric, conditionCallback) {
      /**
    ...
    key: "_getListOfMetrics",
    value: function _getListOfMetrics(h, primaryMetric) {
    ...
    value: function getPrePrunedTrees() {
      ...
      try {
        for (_iterator10.s(); !(_step10 = _iterator10.n()).done;) {
          ...
          var t = tree.copy();
          t.size = t.descendants().length;
          var t_nodes = t.descendants();
          var tree_nodes = tree.descendants();
    ...
    key: "getFreshTrees",
    value: function getFreshTrees() {
      /**
       * Yields a list of un-pruned trees to the calling function.
       */
      var mutableTrees = [];

      var _iterator11 = _createForOfIteratorHelper(this.immutableTrees),
          _step11;
    ...
    key: "getCurrentTree",
    value: function getCurrentTree(index) {
      return this.mutableTrees[index];
    }
  }, {
    key: "setCurrentTree",
    value: function setCurrentTree(index, tree) {
...
/* harmony default export */ const cct_repr = (Forest);
...
;// CONCATENATED MODULE: ./scripts/cct/cct_model.js
function cct_model_createForOfIteratorHelper(o, allowArrayLike) { var it = typeof Symbol !== "undefined" && o[Symbol.iterator] || o["@@iterator"]; if (!it) { if (Array.isArray(o) || (it = cct_model_unsupportedIterableToArray(o)) || allowArrayLike && o && typeof o.length === "number") { if (it) o = it; var i = 0; var F = function F() {}; return { s: F, n: function n() { if (i >= o.length) return { done: true }; return { done: false, value: o[i++] }; }, e: function e(_e) { throw _e; }, f: F }; } throw new TypeError("Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); } var normalCompletion = true, didErr = false, err; return { s: function s() { it = it.call(o); }, n: function n() { var step = it.next(); normalCompletion = step.done; return step; }, e: function e(_e2) { didErr = true; err = _e2; }, f: function f() { try { if (!normalCompletion && it["return"] != null) it["return"](); } finally { if (didErr) throw err; } } }; }

function cct_model_unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return cct_model_arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return cct_model_arrayLikeToArray(o, minLen); }

function cct_model_arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) { arr2[i] = arr[i]; } return arr2; }

function cct_model_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var Model = /*#__PURE__*/function () {
  function Model() {
    cct_model_classCallCheck(this, Model);

    this._observers = makeSignaller(); //initialize default data and state
    ...
    this.forest = new cct_repr(_forestData); // // pick the first metric listed to color the nodes
  ...
  cct_model_createClass(Model, [{
    key: "_initializePruneRange",
    value: function _initializePruneRange(bins) {
      var _this = this;

      bins.forEach(function (d) {
        _this.state.prune_range["low"] = Math.min(d.x0, _this.state.prune_range[0]);
        _this.state.prune_range["high"] = Math.max(d.x1, _this.state.prune_range[1]);
      });
    }
  }, {
    key: "_printNodeData",
    value: function _printNodeData(nodeList) {
      /**
      ...
      var metricColumns = this.forest["metricColumns"]; //lay the nodes out in a table
      ...
      for (var _i = 0; _i < numNodes; _i++) {
        ...
        for (var j = 0; j < metricColumns.length; j++) {
          if (j == 0) {
            if (nodeList[_i].data.aggregate) {
              if (nodeList[_i].data.elided.length == 1) {
                nodeStr += "<td>".concat(nodeList[_i].data.prototype.data.name, " Subtree </td>");
              } else if (nodeList[_i].data.elided.length > 1) {
                nodeStr += "<td>Children of: ".concat(nodeList[_i].parent.data.name, " </td>");
              }
            } else {
              nodeStr += "<td>".concat(nodeList[_i].data.name, "</td>");
            }
          }
    ...
    key: "_printQuery",
    value: function _printQuery(nodeList, exclude) {
      /**
      ...
      var queryStr = '<no query generated>'; // let conditions = ''
      ...
      if (!leftMostNode.data.aggregate && !rightMostNode.data.aggregate) {
        if (nodeList.length > 1 && selectionIsAChain) {
          ...
          queryStr = [{
            name: leftMostNode.data.frame.name
          }, '*', {
            name: rightMostNode.data.frame.name
          }];
    ...
    key: "updateBins",
    value: function updateBins(numBins) {
      var _this2 = this;
      ...
      if (this.data.distCounts.length != numBins) {
        var _iterator = cct_model_createForOfIteratorHelper(this.forest.getTrees()),
            _step;

        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            var t = _step.value;
            nodes = nodes.concat(t.descendants().filter(function (x) {
              return x.data.metrics != undefined;
            }));
          }
        } catch (err) {
          _iterator.e(err);
      ...
      var bins = bin().value(function (d) {
        return d.data.metrics[_this2.state.primaryMetric];
      }).thresholds(numBins);
      var bin_dist = bins(nodes);
      var zero_cnts = new Array(bin_dist.length).fill().map(function (d) {
        return [];
      });
      var b = 0;
      var x0 = bin_dist[b].x0;
      var x1 = bin_dist[b].x1;
      bin_dist[b] = bin_dist[b].filter(function (value) {
        return value.data.metrics[_this2.state.primaryMetric] != 0;
      });
      bin_dist[b]['x0'] = x0;
      bin_dist[b]['x1'] = x1; //store zero metric internal nodes

      var _loop = function _loop(bn) {
        var parent = null;

        var _iterator2 = cct_model_createForOfIteratorHelper(bin_dist[bn]),
            _step2;
        ...
        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
            ...
            while (parent !== null) {
              if (zero_cnts[bn].some(function (node) {
                return node.data.id == parent.data.id;
              })) {
                break;
              } else if (parent.data.metrics[_this2.state.primaryMetric] == 0) {
      ...
      this.data.distCounts = {
        "nonzero": bin_dist,
        "internalzero": zero_cnts
      };
    }
  }, {
    key: "enablePruneTree",
    value: function enablePruneTree(threshold) {
      /**
       ...
       */
      this.state.pruneEnabled = !this.state.pruneEnabled; // if (this.state.pruneEnabled){
    ...
    key: "_setRangeFlags",
    value: function _setRangeFlags() {
      /**
       * Sets a show flag when a metric is inside of the provided range.
       */
      var mdl = this;
      return function (tree, primaryMetric) {
    ...
    key: "_setOutlierFlags",
    value: function _setOutlierFlags() {
      /**
      ...
      return function (h, primaryMetric) {
        ...
        h.each(function (node) {
          ...
          if (metric >= upperOutlierThreshold || metric <= lowerOutlierThreshold) {
            node.data.show = 1;
          } else {
            node.data.show = 0;
          }
    ...
    key: "pruneTree",
    value: function pruneTree(threshold) {
      /**
    ...
    key: "updateSelected",
    value: function updateSelected(nodes) {
      /**
    ...
    value: function handleDoubleClick(d) {
      /**
      ...
      if (!d.data.aggregate) {
        if (d.parent) {
          ...
          var dummy = this.forest._buildDummyHolder(d, d.parent, [d]);

          var swapIndex = d.parent.children.indexOf(d);
      ...
      // Appends if multiple were elided
        ...
        if (d.data.elided.length == 1) {
          ...
          var insIndex = d.parent.children.indexOf(d);
          d.parent.children[insIndex] = d.data.prototype;
        } else {
          var _iterator3 = cct_model_createForOfIteratorHelper(d.data.elided),
              _step3;

          try {
            for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
              var elided = _step3.value;
              delete elided.data.aggregateMetrics;

              var delIndex = d.parent._children.indexOf(elided);
          ...
          d.parent.children = d.parent.children.filter(function (child) {
            return child !== d;
          });
        }
      }
    ...
    key: "setBrushedPoints",
    value: function setBrushedPoints(selection) {
      /**
    ...
    key: "updateTooltip",
    value: function updateTooltip(nodes) {
      /**
    ...
    key: "changeMetric",
    value: function changeMetric(newMetric, source) {
      /**
    ...
    value: function changeColorScheme(v) {
      /**
    ...
    key: "updateLegends",
    value: function updateLegends(v) {
      /**
    ...
    key: "updateActiveTrees",
    value: function updateActiveTrees(activeTree) {
      /**
    ...
    key: "updatePruneRange",
    value: function updatePruneRange(low, high) {
      /**
      ...
      this.forest.aggregateTreeData(this.state.primaryMetric, function (h, primaryMetric) {
        h.each(function (node) {
          var metric = node.data.metrics[primaryMetric];
    ...
    key: "toggleMenuActive",
    value: function toggleMenuActive() {
      /**
    ...
    value: function storeSnapshotQuery() {
      ...
      var aggs = leaves.filter(function (n) {
        return n.data.aggregate;
      });
        ...
        for (_iterator8.s(); !(_step8 = _iterator8.n()).done;) {
          ...
          if (initial_flag) {
            if (leaf.data.aggregate) {
              path_query += "WHERE n.\\\"node_id\\\" = ".concat(leaf.data.prototype.data.metrics._hatchet_nid);
            } else {
            ...
            path_query += " OR n.\\\"node_id\\\" = ".concat(leaf.data.metrics._hatchet_nid);
          }
        }
      } catch (err) {
        _iterator8.e(err);
        ...
        try {
          for (_iterator9.s(); !(_step9 = _iterator9.n()).done;) {
            ...
            if (initial_flag) {
              ...
              ex_query += " AND NOT r.\\\"node_id\\\" = ".concat(node.data.metrics._hatchet_nid);
            }
          }
        } catch (err) {
          _iterator9.e(err);
...
/* harmony default export */ const cct_model = (Model);
;// CONCATENATED MODULE: ./scripts/utils/view.js
...
function cct_menu_view_unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return cct_menu_view_arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return cct_menu_view_arrayLikeToArray(o, minLen); }

function cct_menu_view_arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) { arr2[i] = arr[i]; } return arr2; }

function cct_menu_view_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
function _possibleConstructorReturn(self, call) { if (call && (cct_menu_view_typeof(call) === "object" || typeof call === "function")) { return call; } else if (call !== void 0) { throw new TypeError("Derived constructors may only return object or undefined"); } return _assertThisInitialized(self); }
...
var MenuView = /*#__PURE__*/function (_View) {
   ...
   */
  function MenuView(elem, model) {
    var _this;

    cct_menu_view_classCallCheck(this, MenuView);

    _this = _super.call(this, elem, model);
    _this._svg = null;
    _this._svgButtonOffset = 0;
    _this.categories = ['Metrics', 'Display', 'Query'];
    _this.model_var_map = {};
    _this.menu_tree = [];
    _this.width = elem.clientWidth - globals.layout.margin.right - globals.layout.margin.left;
    _this.height = globals.treeHeight * (model.forest.numberOfTrees + 1);
    _this.menu_height = '2em';
    _this.menu_bg_color = 'rgba(100,100,100,1)'; //state things

    _this.prior_submenu = null;
    _this.model.state.menu_active = false;

    _this._setUpMenuTree();

    _this._renderMenuBar();
  ...
  cct_menu_view_createClass(MenuView, [{
    key: "_makeSubmenuOption",
    value: function _makeSubmenuOption(text, type, options, model_var, callback) {
      /**
    ...
    value: function _setUpMenuTree() {
      ...
      var rootNodeNames = model.forest.rootNodeNames;
      var metricColumns = model.forest.metricColumns;
      ...
      this.menu_tree.Metrics.push(this._makeSubmenuOption('Color (Primary Metric)', 'dropdown', metricColumns, 'primaryMetric', function (evt_sel, val) {
      ...
      this.menu_tree.Metrics.push(this._makeSubmenuOption('Size (Secondary Metric)', 'dropdown', metricColumns, 'secondaryMetric', function (evt_sel, val) {
        self.observers.notify({
          type: globals.signals.METRICCHANGE,
          newMetric: val,
          source: "secondary"
      ...
      this.menu_tree.Display.push(this._makeSubmenuOption('Tree Select', 'dropdown', rootNodeNames, 'activeTree', function (evt_sel, val) {
        self.observers.notify({
          type: globals.signals.TREECHANGE,
          display: val
        });
      }));
      this.menu_tree.Display.push(this._makeSubmenuOption('Color Map', 'dropdown', colors, 'colorText', function (evt_sel, val) {
        self.observers.notify({
          type: globals.signals.COLORCLICK,
          value: val
        });
      }));
      this.menu_tree.Display.push(this._makeSubmenuOption('Legends', 'dropdown', legends, 'legendText', function (evt_sel, val) {
        self.observers.notify({
          type: globals.signals.LEGENDCLICK,
          value: val
        });
      }));
      this.menu_tree.Display.push(this._makeSubmenuOption('Reset View', 'button', null, null, function (evt_sel) {
        self.observers.notify({
          type: globals.signals.RESETVIEW
        });
      })); //add query/filter

      this.menu_tree['Query'].push(this._makeSubmenuOption('Select Nodes', 'toggle', null, 'brushOn', function (evt_sel) {
        self.observers.notify({
          type: globals.signals.TOGGLEBRUSH
        });
      }));
      this.menu_tree['Query'].push(this._makeSubmenuOption('Mass Prune', 'toggle', null, 'pruneEnabled', function (evt_sel) {
        self.observers.notify({
          type: globals.signals.ENABLEMASSPRUNE,
          threshold: 1.5
        });
      }));
      this.menu_tree['Query'].push(this._makeSubmenuOption('Get Snapshot Query', 'button', null, null, function (evt_sel) {
    ...
    key: "_addNewMenuButton",
    value: function _addNewMenuButton(id, text, click) {
      /**
      ...
      var button = this._svg.append('g').attr('id', id).append('rect').attr('height', '15px').attr('x', this._svgButtonOffset).attr('y', 0).attr('rx', 5).style('fill', '#ccc').on('click', click);

      var buttonText = build_d3.select(this.elem).select('#' + id).append('text').attr("x", this._svgButtonOffset + textPad).attr("y", 12).text(text).attr("font-family", "sans-serif").attr("font-size", "12px").attr('cursor', 'pointer').on('click', click);
      var width = buttonText.node().getBBox().width + 2 * textPad;
      button.attr('width', width);
      this._svgButtonOffset += width + buttonPad;
    }
  }, {
    key: "_handleSubmenuVisibility",
    value: function _handleSubmenuVisibility(d) {
      var self = this;
      var submenu = build_d3.select(self.elem).select(".".concat(d, "-submenu"));
    ...
    value: function _renderMenuBar() {
      ...
      // 2em tall, full width
      var menu_svg = build_d3.select(this.elem).append("svg").attr("class", "menu-svg");
      this._svg = menu_svg;
      var self = this;
      var buttonPad = 10;
      var bg_color = this.menu_bg_color;
      var menu_height = '2em';
      var text_v_offset = '1.4em';
      var vis_name_text = 'Interactive Calling Context Tree';
      menu_svg.attr('width', this.elem.clientWidth).attr('height', menu_height);
      menu_svg.append('rect').attr('width', this.elem.clientWidth).attr('height', menu_height).style('fill', bg_color);
      var vis_name = menu_svg.append('text').text(vis_name_text).attr('x', this.elem.clientWidth).attr('y', 20).attr("font-family", "sans-serif").attr("font-size", "14px").style('fill', 'rgba(256,256,256,1)');
      vis_name.attr('x', function () {
        return self.elem.clientWidth - build_d3.select(this).node().getBBox().width - 25;
      });
      var options = menu_svg.selectAll('.option').data(this.categories);
      var op_grp = options.enter().append('g').attr('class', 'option').attr('cursor', 'pointer').on('mouseover', function (d) {
        build_d3.select(this).select('.menu-button').style('fill', 'rgba(150,150,150,1)');
        ...
        build_d3.select(this).select('.menu-button').style('fill', bg_color);
      }).on('click', function (d) {
        self.observers.notify({
      ...
      op_grp.append('rect').attr('class', 'menu-button').attr('height', menu_height).style('fill', bg_color);
      op_grp.append('text').text(function (d) {
        return d;
      }).attr('class', 'menu-text').attr('y', text_v_offset).attr('x', buttonPad).attr("font-family", "sans-serif").attr("font-size", "14px").style('fill', 'rgba(256,256,256,1)');
      var offset = 0;
      op_grp.each(function (d) {
        var btn_grp = build_d3.select(this);
        var btn_width = btn_grp.select(".menu-text").node().getBBox().width + 2 * buttonPad; //apply modified widths and offsets

        btn_grp.select(".menu-button").attr('width', btn_width).style('stroke', 'black').style('stroke-dasharray', "0, ".concat(btn_width, ", ").concat(menu_height, ", ").concat(btn_width, ", ").concat(menu_height, ", 0"));
        btn_grp.attr('transform', "translate(".concat(offset, ", 0)"));
    ...
    key: "_addSubmenu",
    value: function _addSubmenu(submenu_name, submenu_options, x_offset) {
      /**
       * Renders a submenu drop down under a top level menu button.
       */
      var view_left = this.elem.getBoundingClientRect().left - this.elem.parentNode.getBoundingClientRect().left;

      var view_top = this._svg.select('rect').node().getBBox().height;

      var button_pad = 10;
      var self = this;
      var submenu_window = build_d3.select(this.elem).append('svg').style('position', 'absolute').style('top', view_top + 16 + 'px').style('left', view_left + x_offset + 5 + 'px').style('width', '600px').attr('class', "".concat(submenu_name, "-submenu")).style('visibility', 'hidden').append('g').attr('class', 'submenu-grp');
      var border = submenu_window.append('rect').attr('height', submenu_options.length * 25).style('stroke', 'white').style('stroke-width', 2);
      var opt = submenu_window.selectAll('.submenu-button').data(submenu_options);
      var btn = opt.enter().append('g').attr('class', 'submenu-button').attr('transform', function (_, i) {
        return "translate(0, ".concat(i * 25 + 1, ")");
      }).attr('cursor', 'pointer').on('mouseover', function () {
        build_d3.select(this).select('.submenu-button-rect').style('fill', 'rgba(150,150,150,1)');
      }).on('mouseout', function () {
        build_d3.select(this).select('.submenu-button-rect').style('fill', self.menu_bg_color);
      }).on('click', function (d) {
        if (d.type != 'dropdown') d.onclick(this);
      });
      var bar = btn.append('rect').attr('class', 'submenu-button-rect').attr('height', 25).style('fill', this.menu_bg_color);
      btn.append('text').text(function (d) {
        return d['text'];
      }).attr('class', 'submenu-button-text').attr('y', '1.2em').attr('x', button_pad).attr("font-family", "sans-serif").attr("font-size", "14px").style('fill', 'rgba(256,256,256,1)');
      var max_width = 0;
      btn.each(function (d) {
        max_width = Math.max(this.getBBox().width, max_width);
      });
      var barwidth = max_width + 2 * button_pad + 25;
      bar.attr('width', barwidth);
      border.attr('width', barwidth);
      btn.each(function (d) {
        var this_button = build_d3.select(this);

        if (d.type == 'dropdown') {
          self._makeDropDownMenu(this_button, d.options, barwidth, d.onselect);

          this_button.on('mouseenter', function () {
            var submenu = build_d3.select(this).select(".cct-dropdown-menu");
            submenu.style('visibility', 'visible');
      ...
      btn.append('text').text(function (d) {
        if (d.type == 'dropdown') return '▸';
      }).attr('class', 'submenu-icon').attr('y', '1.2em').attr('x', max_width + 25).attr("font-family", "sans-serif").attr("font-size", "14px").style('fill', 'rgba(256,256,256,1)');
    }
  }, {
    key: "_makeDropDownMenu",
    value: function _makeDropDownMenu(button, options, xoffset, callback) {
      /**
       * Renders a list of options under a 'dropdown' submenu option.
       */
      var xorigin = xoffset;
      var yorigin = button.node().getBBox().y;
      var button_pad = 10;
      var self = this;
      var selections = button.append('g').attr('class', 'cct-dropdown-menu').attr('transform', "translate(".concat(xorigin, ", ").concat(yorigin, ")")).style('visibility', 'hidden');
      selections.append('rect').attr('height', options.length * 25).attr('width', 150).style('stroke', 'white').style('stroke-width', 2);
      var sel = selections.selectAll('.cct-dropdown-option').data(options);
      var opt = sel.enter().append('g').attr('class', 'cct-dropdown-option').attr('transform', function (_, i) {
        return "translate(0, ".concat(i * 25, ")");
      }).attr('cursor', 'pointer').on('mouseenter', function () {
        build_d3.select(this).select('.cct-dropdown-option-rect').style('fill', 'rgba(150,150,150,1)');
      }).on('mouseleave', function () {
        build_d3.select(this).select('.cct-dropdown-option-rect').style('fill', self.menu_bg_color);
      }).on('click', function (d) {
        callback(this, d);
      });
      opt.append('rect').attr('height', 25).attr('width', 150).attr('class', 'cct-dropdown-option-rect').style('fill', this.menu_bg_color);
      opt.append('text').text(function (d) {
        return d;
      }).attr('class', 'cct-dropdown-option-text').attr('y', '1.2em').attr('x', button_pad).attr("font-family", "sans-serif").attr("font-size", "14px").style('fill', 'rgba(256,256,256,1)');
      opt.append('text').text(function (_, i) {
        if (i == 0) return '✓';
      }).attr('class', 'cct-dropdown-icon').attr('y', '1.2em').attr('x', 130).attr("font-family", "sans-serif").attr("font-size", "16px").style('fill', 'rgba(256,256,256,1)');
    }
  }, {
    key: "_preRender",
    value: function _preRender() {
      ...
      var brush = build_d3.brush().extent([[0, 0], [2 * this.width, 2 * (this.height + globals.layout.margin.top + globals.layout.margin.bottom)]]).on('brush', function () {
        self.observers.notify({
          type: globals.signals.BRUSH,
          selection: build_d3.event.selection,
          end: false
        });
      }).on('end', function () {
        self.observers.notify({
      ...
      build_d3.select("#mainG").append('g').attr('class', 'brush').call(brush);
      this.brush = brush;
    }
  }, {
    key: "render",
    value: function render() {
      ...
      var _iterator = cct_menu_view_createForOfIteratorHelper(this.categories),
          _step;

      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          var option = _step.value;
          var submenuopts = build_d3.select(this.elem).select(".".concat(option, "-submenu")).selectAll('.submenu-button');
          submenuopts.each(function (d) {
            var dropdownopts = build_d3.select(this).selectAll(".cct-dropdown-option");
            dropdownopts.selectAll('.cct-dropdown-icon').text(function (v) {
            ...
            if (d.type == 'toggle') {
              if (pruneEnabled && d.text == 'Mass Prune') {
                build_d3.select(this).select('.submenu-icon').text('✓');
      ...
      build_d3.select(this.elem).selectAll('.brush').remove(); //add brush if there should be one

      if (brushOn > 0) {
        this._svg.select("#mainG").append('g').attr('class', 'brush').call(this.brush);
      }
    }
  }]);
...
/* harmony default export */ const cct_menu_view = (MenuView);
;// CONCATENATED MODULE: ./scripts/cct/cct_color_manager.js
function cct_color_manager_createForOfIteratorHelper(o, allowArrayLike) { var it = typeof Symbol !== "undefined" && o[Symbol.iterator] || o["@@iterator"]; if (!it) { if (Array.isArray(o) || (it = cct_color_manager_unsupportedIterableToArray(o)) || allowArrayLike && o && typeof o.length === "number") { if (it) o = it; var i = 0; var F = function F() {}; return { s: F, n: function n() { if (i >= o.length) return { done: true }; return { done: false, value: o[i++] }; }, e: function e(_e) { throw _e; }, f: F }; } throw new TypeError("Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); } var normalCompletion = true, didErr = false, err; return { s: function s() { it = it.call(o); }, n: function n() { var step = it.next(); normalCompletion = step.done; return step; }, e: function e(_e2) { didErr = true; err = _e2; }, f: function f() { try { if (!normalCompletion && it["return"] != null) it["return"](); } finally { if (didErr) throw err; } } }; }

function cct_color_manager_unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return cct_color_manager_arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return cct_color_manager_arrayLikeToArray(o, minLen); }

function cct_color_manager_arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) { arr2[i] = arr[i]; } return arr2; }

function cct_color_manager_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var ColorManager = /*#__PURE__*/function () {
   ...
   * @param {int} treeIndex 
   */
  function ColorManager(model, treeIndex) {
    cct_color_manager_classCallCheck(this, ColorManager);

    var REGULAR_COLORS = [['#006d2c', '#31a354', '#74c476', '#a1d99b', '#c7e9c0', '#edf8e9'], //green
    ...
    var CAT_COLORS = ["#3366cc", "#dc3912", "#ff9900", "#109618", "#990099", "#0099c6", "#dd4477", "#66aa00", "#b82e2e", "#316395", "#994499", "#22aa99", "#aaaa11", "#6633cc", "#e67300", "#8b0707", "#651067", "#329262", "#5574a6", "#3b3eac"];
    this._regularColors = {
      0: REGULAR_COLORS.map(function (colorArr) {
        return [].concat(colorArr).reverse();
      }),
      1: REGULAR_COLORS,
      2: CAT_COLORS,
      3: [].concat(CAT_COLORS).reverse()
    };
    var ALL_COLORS = ['#d73027', '#fc8d59', '#fee090', '#e0f3f8', '#91bfdb', '#4575b4'];
    ...
    this.universal_color_scale = build_d3.scaleQuantize().domain([this._forestMinMax[this._state.primaryMetric].min, this._forestMinMax[this._state.primaryMetric].max]).range(this._allTreesColors[this._state.colorScheme]);
    this.single_color_scale = build_d3.scaleQuantize().domain([this._forestStats[this.treeIndex][this._state.primaryMetric].min, this._forestStats[this.treeIndex][this._state.primaryMetric].max]).range(this._regularColors[this._state.colorScheme][this.treeIndex]);
    this.agg_color_scale = build_d3.scaleQuantize().domain([this._aggregateMinMax[this._state.primaryMetric].min, this._aggregateMinMax[this._state.primaryMetric].max]).range(this._allTreesColors[this._state.colorScheme]);
  }

  cct_color_manager_createClass(ColorManager, [{
    key: "_updateScales",
    value: function _updateScales() {
       ...
       */
      var a_dom = this._aggregateMinMax[this._state.primaryMetric];
      this.agg_color_scale.domain([a_dom.min, a_dom.max]);

      if (this._state.primaryMetric != this.cachedPrimaryMetric) {
        var u_dom = this._forestMinMax[this._state.primaryMetric];
        var s_dom = this._forestStats[this.treeIndex][this._state.primaryMetric];
    ...
    value: function _getDomainFromScale(scale) {
      /**
      ...
      var _iterator = cct_color_manager_createForOfIteratorHelper(scale.range()),
          _step;
    ...
    key: "_getCorrectScale",
    value: function _getCorrectScale() {
      /**
    ...
    key: "getAggLegendDomains",
    value: function getAggLegendDomains() {
      /**
    ...
    key: "getLegendDomains",
    value: function getLegendDomains() {
      /**
    ...
    key: "calcColorScale",
    value: function calcColorScale(nodeData) {
      /**
       * Update the scales for the current primary metric.
       */
      this._updateScales();

      var nodeMetric = nodeData.metrics[this._state.primaryMetric];
      return this._getCorrectScale()(nodeMetric);
    }
  }, {
    key: "calcAggColorScale",
    value: function calcAggColorScale(nodeData) {
      /**
      * Update aggregate scales for the current primary metric.
      */
      this._updateScales();

      var nodeMetric = nodeData.aggregateMetrics[this._state.primaryMetric];
      return this._getCorrectScale()(nodeMetric);
    }
  }, {
    key: "getColorLegend",
    value: function getColorLegend() {
      this._updateScales();

      return this._getCorrectScale().range();
    }
  }, {
    key: "getAggColorLegend",
    value: function getAggColorLegend() {
      this._updateScales();
...
var makeColorManager = function makeColorManager(model) {
  // TODO: Move the colors to a color.js.
  var color_scale = d3.scaleQuantize();
  return {
    setColors: function setColors(treeIndex) {
      var curMetric = _state["primaryMetric"];
      var colorScheme = _state["colorScheme"];

      if (_metricColumns.includes(curMetric)) {
        if (treeIndex == -1) return _allTreesColors[colorScheme];else return _regularColors[colorScheme][treeIndex % REGULAR_COLORS.length];
      } else if (_attributeColumns.includes(curMetric)) {
        if (treeIndex == -1) return _allTreesColors[2 + colorScheme];else return _regularColors[2 + colorScheme];
      }
    },
    getLegendDomains: function getLegendDomains(treeIndex) {
      /**
       * Sets the min and max of the legend. 
       * 
       * @param {Int} treeIndex - The index of the current tree's legend being set
       */
      var curMetric = _state["primaryMetric"]; // so hacky: need to fix later
      ...
      if (_metricColumns.includes(curMetric)) {
        var metricRange = metricMinMax.max - metricMinMax.min;
    ...
    getColorLegend: function getColorLegend(treeIndex) {
      /**
    ...
    calcColorScale: function calcColorScale(nodeData, treeIndex) {
      /**
      ...
      var curMetric = _state["primaryMetric"]; // Get the suitable data based on the Legend settings.
      ...
      if (_attributeColumns.includes(curMetric)) {
        var nodeMetric = nodeData.attributes[curMetric];

        var indexOfMetric = _d.indexOf(nodeMetric);

        return colorSchemeUsed[indexOfMetric];
      } else if (_metricColumns.includes(curMetric)) {
        var _nodeMetric = nodeData.metrics[curMetric];
        color_scale.domain([_d.min, _d.max]).range(colorSchemeUsed);
        return color_scale(_nodeMetric);
      }
    }
  };
};
...
function cct_chart_view_possibleConstructorReturn(self, call) { if (call && (cct_chart_view_typeof(call) === "object" || typeof call === "function")) { return call; } else if (call !== void 0) { throw new TypeError("Derived constructors may only return object or undefined"); } return cct_chart_view_assertThisInitialized(self); }

function cct_chart_view_assertThisInitialized(self) { if (self === void 0) { throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); } return self; }
...
var Legend = /*#__PURE__*/function (_View) {
  cct_chart_view_inherits(Legend, _View);

  var _super = cct_chart_view_createSuper(Legend);

  /**
   * Handles the drawing and management of different types of legends.
   */
  function Legend(elem, model, encoding_type, tree_index, cm, nodeScale) {
    var _this;

    cct_chart_view_classCallCheck(this, Legend);

    _this = _super.call(this, elem, model);
    _this._colorManager = cm;
    _this._nodeScale = nodeScale;
    _this.metricColumns = model.forest.metricColumns;
    _this.attributeColumns = model.forest.attributeColumns;
    _this.tree_index = tree_index;
    _this.secondaryMinMax = _this.model.forest.forestMinMax;
    _this.type = encoding_type;
    _this.agg = false;
    _this.leg_grp = elem.append('g').attr('class', 'legend-grp');
    _this.quantNodeScale = build_d3.scaleQuantize().range([0, 1, 2, 3, 4, 5]).domain([_this.secondaryMinMax[_this.model.state.secondaryMetric].min, _this.secondaryMinMax[_this.model.state.secondaryMetric].max]);
  ...
  cct_chart_view_createClass(Legend, [{
    key: "setAgg",
    value: function setAgg() {
      this.agg = true;
    }
  }, {
    key: "offset",
    value: function offset(x) {
      /**
       * Offsets a legend by x pixels along the x axis
       */
      this.leg_grp.attr('transform', "translate(".concat(x, ", 0)"));
    }
  }, {
    key: "getLegendWidth",
    value: function getLegendWidth() {
      return this.leg_grp.node().getBBox().width;
    }
  }, {
    key: "getLegendHeight",
    value: function getLegendHeight() {
    ...
    value: function preRender() {
      ...
      this.leg_grp.append('text').text(function () {
        if (_this2.type == 'color') {
          return "Legend for metric: ".concat(_this2.model.state.primaryMetric);
        } else if (_this2.type == 'radius') {
          return "Legend for metric: ".concat(_this2.model.state.secondaryMetric);
        }
      }).attr('class', 'legend-title').attr('x', '-2em').attr('y', '-1em').attr('font-family', 'sans-serif').attr('font-size', '14px');
      var legendGroups = this.leg_grp.selectAll("g").data([0, 1, 2, 3, 4, 5]).enter().append('g').attr('class', 'legend-lines').attr('transform', function (_, i) {
        if (_this2.type == 'color') {
          var y = 18 * i;
      ...
      if (this.type == 'color') {
        legendGroups.append('rect').attr('class', 'legend-samples').attr('x', 0).attr('y', 0).attr('height', 15).attr('width', 10).style('stroke', 'black');
        legendGroups.append('text').attr('class', 'legend-ranges').attr('x', 12).attr('y', 13).text("0.0 - 0.0").style('font-family', 'monospace').style('font-size', '12px');
      }
    ...
    value: function render() {
      ...
      var leg_dom = this._colorManager.getLegendDomains(this.tree_index);

      this.leg_grp.selectAll(".legend-title").text(function () {
        if (_this3.type == 'color') {
          return "Legend for metric: ".concat(_this3.model.state.primaryMetric);
        } else if (_this3.type = 'radius') {
          return "Legend for metric: ".concat(_this3.model.state.secondaryMetric);
        }
      });
      this.leg_grp.selectAll(".legend-samples").transition().duration(globals.duration).attr('fill', function (d) {
        if (_this3.type == "color") {
          return _this3._colorManager.getColorLegend(_this3.tree_index)[5 - d];
        }

        return 'white';
      }).attr('stroke', 'black');
      this.leg_grp.selectAll('.legend-ranges').transition().duration(globals.duration).text(function (_, i) {
        if (_this3.type == "color") {
          if (_this3.metricColumns.includes(_this3.model.state["primaryMetric"])) {
            return getSigFigString(leg_dom[5 - i][0]) + ' - ' + getSigFigString(leg_dom[5 - i][1]);
          } else if (_this3.attributeColumns.includes(_this3.model.state["primaryMetric"])) {
            return leg_dom[i];
          }
        } else if (_this3.type == "radius") {
          var range = _this3.quantNodeScale.invertExtent(5 - i);

          return "".concat(getSigFigString(range[0]), " - ").concat(getSigFigString(range[1]));
        }
      });
    }
  }]);
...
var ChartView = /*#__PURE__*/function (_View2) {
  cct_chart_view_inherits(ChartView, _View2);

  var _super2 = cct_chart_view_createSuper(ChartView);

  function ChartView(elem, model) {
    var _this4;

    cct_chart_view_classCallCheck(this, ChartView);

    _this4 = _super2.call(this, elem, model); //layout variables

    _this4._margin = globals.layout.margin;
    ...
    _this4.treeOffset = 0;
    _this4._minmax = [];
    _this4.svg = build_d3.select(elem).append('svg').attr("class", "canvas").attr("width", _this4._width).attr("height", _this4._height);
    var fMaxHeight = model.forest.maxHeight;
    var secondaryMinMax = model.forest.forestMinMax[model.state.secondaryMetric]; //scales
    ...
    _this4.aggregates = [];
    _this4.links = [];
    _this4.metricColumns = model.forest.metricColumns;
    _this4.attributeColumns = model.forest.attributeColumns;
    _this4.primary_legends = [];
    _this4.secondary_legends = [];
    _this4.color_managers = [];
    _this4.newDataFlag = 1;
  ...
  cct_chart_view_createClass(ChartView, [{
    key: "checkCollison",
    value: function checkCollison(box1, box2) {
      /**
    ...
    key: "manageLabelCollisions",
    value: function manageLabelCollisions(nodes) {
      var _this5 = this;
      ...
      var self = this;
      var testBBs = []; //load bounding boxes 
    ...
    key: "diagonal",
    value: function diagonal(s, d, ti) {
      /**
      ...
      var path = "M ".concat(sy, " ").concat(sx, "\n        C ").concat((sy + dy) / 2, " ").concat(sx, ",\n        ").concat((sy + dy) / 2, " ").concat(dx, ",\n        ").concat(dy, " ").concat(dx);
      return path;
    }
  }, {
    key: "_getLocalNodeX",
    value: function _getLocalNodeX(x, ti) {
      /**
    ...
    key: "_getMinxMaxxFromTree",
    value: function _getMinxMaxxFromTree(root) {
      /**
    ...
    key: "_getHeightFromTree",
    value: function _getHeightFromTree(root) {
      /**
       ...
       * @param {Object} root - The root node of the working tree
       */
      var minmax = this._getMinxMaxxFromTree(root);
    ...
    value: function _getSelectedNodes(selection) {
      /**
      ...
      if (selection) {
        for (var i = 0; i < this.model.forest.numberOfTrees; i++) {
          this.nodes[i].forEach(function (d) {
            if (selection[0][0] <= d.yMainG && selection[1][0] >= d.yMainG && selection[0][1] <= d.xMainG && selection[1][1] >= d.xMainG) {
              brushedNodes.push(d);
            }
          });
        }
    ...
    key: "_calcNodePositions",
    value: function _calcNodePositions(nodes, treeIndex) {
      var _this6 = this;
       ...
       */
      nodes.forEach(function (d) {
        d.x0 = _this6._getLocalNodeX(d.x, treeIndex);
        d.y0 = _this6._treeDepthScale(d.depth); // Store the overall position based on group

        d.xMainG = d.x0 + _this6.chartOffset;
        d.yMainG = d.y0 + _this6._margin.left;
      });
    }
  }, {
    key: "_preRender",
    value: function _preRender() {
      ...
      this.zoom = build_d3.zoom().on("zoom", function () {
        var zoomObj = build_d3.select(this).selectAll(".chart");
        zoomObj.attr("transform", build_d3.event.transform);
      }).on("end", function () {
        var zoomObj = build_d3.select(this).selectAll(".chart");
        var index = zoomObj.attr("chart-id");
        var transformation = zoomObj.node().getCTM();
        self.nodes[index].forEach(function (d) {
          /**
      ...
      for (var treeIndex = 0; treeIndex < this.model.forest.numberOfTrees; treeIndex++) {
        var layout = build_d3.tree().nodeSize([this._maxNodeRadius + 4, this._maxNodeRadius + 4]); // .size([this._treeCanvasHeightScale(this.model.forest.getCurrentTree(treeIndex).size), this._width - this._margin.left - 200]);

        var tree = this.model.forest.getCurrentTree(treeIndex);
        var currentRoot = layout(tree);

        var currentLayoutHeight = this._getHeightFromTree(currentRoot);

        var currentMinMax = this._getMinxMaxxFromTree(currentRoot);

        var newg = mainG.append("g").attr('class', 'group-' + treeIndex + ' subchart').attr('tree_id', treeIndex).attr("transform", "translate(" + this._margin.left + "," + this.chartOffset + ")");
        var cm = new cct_color_manager(this.model, treeIndex);
        var primary_legend = new Legend(newg, this.model, 'color', treeIndex, cm, this._nodeScale);
        var secondary_legend = new Legend(newg, this.model, 'radius', treeIndex, cm, this._nodeScale);
        ...
        this.treeOffset = 0 + this.legendOffset + this._margin.top; //make an invisible rectangle for zooming on

        newg.append('rect').attr('class', 'zoom-rect').attr('height', currentLayoutHeight + this.treeOffset).attr('width', this._width).attr('fill', 'rgba(0,0,0,0)'); //put tree itself into a group

        newg.append('g').attr('class', 'chart').attr('chart-id', treeIndex).attr('height', globals.treeHeight).attr('width', this._width).attr('fill', 'rgba(0,0,0,0)');
        newg.style("display", "inline-block"); //store node and link layout data for use later
        ...
        this.links.push(treeLayout.descendants().slice(1)); //storage

        this._treeLayoutHeights.push(currentLayoutHeight);

        this._minmax.push(currentMinMax); //updates
        ...
        newg.call(this.zoom).on("dblclick.zoom", null);
      } //Update the height


      this.svg.attr("height", this._height); //setup Interactions

      this.brush = build_d3.brush().extent([[0, 0], [2 * this._width, 2 * (this._height + globals.layout.margin.top + globals.layout.margin.bottom)]]).on('brush', function () {}).on('end', function () {
        var selection = _this7._getSelectedNodes(build_d3.event.selection);
    ...
    value: function render() {
      ...
      this._height = this._margin.top + this._margin.bottom; //update scales

      this._nodeScale.domain([0, this.model.forest.forestMinMax[this.model.state.secondaryMetric].max]); // this._aggNodeScale.domain([this.model.forest.aggregateMinMax[this.model.state.secondaryMetric].min, this.model.forest.aggregateMinMax[this.model.state.secondaryMetric].max]);
      //add brush if there should be one


      if (this.model.state.brushOn > 0) {
        this.svg.select("#mainG").append('g').attr('class', 'brush').call(this.brush);
      ...
      var _loop = function _loop() {
        ...
        if (_this8.model.state.hierarchyUpdated == true) {
          // let layout = d3.tree().size([this._treeCanvasHeightScale(source.size), this._width - this._margin.left - 200]);
          var layout = build_d3.tree().nodeSize([_this8._maxNodeRadius + 4, _this8._maxNodeRadius + 4]);
          treeLayout = layout(source);
          _this8.nodes[treeIndex] = treeLayout.descendants().filter(function (d) {
            return !d.data.aggregate;
          });
          _this8.aggregates[treeIndex] = treeLayout.descendants().filter(function (d) {
            return d.data.aggregate;
          });
          _this8.links[treeIndex] = treeLayout.descendants().slice(1); //recalculate layouts

          _this8._treeLayoutHeights[treeIndex] = _this8._getHeightFromTree(treeLayout);
          _this8._minmax[treeIndex] = _this8._getMinxMaxxFromTree(treeLayout); //THIS MUST COME AFTER this._minmax update!!

          _this8._calcNodePositions(treeLayout.descendants(), treeIndex); //only update after last tree


          if (treeIndex == _this8.model.forest.numberOfTrees - 1) {
            _this8.model.state.hierarchyUpdated = false;
          }

          _this8.newDataFlag = 1;
        }

        chart = _this8.svg.selectAll('.group-' + treeIndex);
        treeGroup = chart.selectAll('.chart');

        if (_this8.model.state.resetView == true) {
          /**
           * BUG - D3 TRANSFORM EVENET DOES NOT UPDATE
           */
          treeGroup.attr("transform", "");
          chart.call(_this8.zoom.transform, build_d3.zoomIdentity);

          _this8.nodes[treeIndex].forEach(function (d) {
            // Store the overall position based on group
            d.xMainG = d.x0 + _this8.chartOffset;
            d.yMainG = d.y0 + _this8._margin.left;
          }); //only update after last tree


          if (treeIndex == _this8.model.forest.numberOfTrees - 1) {
            _this8.model.state.resetView = false;
          }
        } // ---------------------------------------------
        // ENTER 
        // ---------------------------------------------
        ...
        standardNodes = treeGroup.selectAll(".node").data(_this8.nodes[treeIndex], function (d) {
          return d.data.metrics._hatchet_nid || d.data.id;
        });
        aggNodes = treeGroup.selectAll(".aggNode").data(_this8.aggregates[treeIndex], function (d) {
          return d.data.metrics._hatchet_nid || d.data.id;
        }); // links

        links = treeGroup.selectAll("path.link").data(_this8.links[treeIndex], function (d) {
          return d.data.metrics._hatchet_nid || d.data.id;
        }); // Enter any new links at the parent's previous position.

        links.enter().append("path").attr("class", "link").attr("d", function (d) {
          return _this8.diagonal(d, d.parent, treeIndex);
        }).attr('fill', 'none').attr('stroke', '#ccc').attr('stroke-width', '2px');
        aggNodeEnter = aggNodes.enter().append('g').attr('class', 'aggNode').attr("transform", function (d) {
          return "translate(".concat(_this8._treeDepthScale(d.depth), ", ").concat(_this8._getLocalNodeX(d.x, treeIndex), ")");
        }).on('mouseover', function (d) {
          var ndgrp = build_d3.select(this);
          ndgrp.selectAll("text").text(function (d) {
            var n = "";

            if (d.data.elided.length == 1) {
              n = "".concat(d.data.prototype.data.name, " Subtree");
            } else if (d.data.elided.length > 1) {
              nodeStr = "Children of: ".concat(d.parent.data.name);
            }
          ...
          var textBBox = ndgrp.select("text").node().getBBox();
          ndgrp.selectAll("rect").attr("visibility", 'visible').attr("width", textBBox.width + 2).attr("height", textBBox.height + 2).attr("x", textBBox.x - 1).attr("y", textBBox.y - 1).attr("stroke-width", "1px").attr("stroke", "rgb(30,30,30)");
        }).on('mouseout', function (d) {
          build_d3.select(this).selectAll("text").text("");
          build_d3.select(this).selectAll("rect").attr("visibility", 'hidden');
        }).on("click", function (d) {
          console.log(d);

          _this8.observers.notify({
        ...
        aggNodeEnter.append("text").attr("x", function (d) {
          ...
          return "";
        }).style("font", "12px monospace").style('fill', 'rgba(0,0,0,.9)'); // Enter any new nodes at the parent's previous position.
        ...
        nodeEnter = standardNodes.enter().append('g').attr('class', 'node').attr("transform", function (d) {
          ...
          if (!(d.data.text === undefined && (!d.children || d.children.length == 0))) {
            ndgrp.selectAll("text").text(function () {
              var n = d.data.name;
        ...
        nodeEnter.append('rect').attr('fill', 'rgba(255,255,255,1)').attr("visibility", 'visible');
        nodeEnter.append("circle").attr('class', 'circleNode').style("fill", function (d) {
          ...
          if (build_d3.event.shiftKey) {
            if (_this8.model.state.selectedNodes.includes(d)) {
              var delndx = _this8.model.state.selectedNodes.indexOf(d);

              _this8.model.state.selectedNodes.splice(delndx, 1);

              data = _this8.model.state.selectedNodes;
            } else {
              data = data.concat(_this8.model.state.selectedNodes);
            }
          }
        ...
        chart.transition().duration(globals.duration).attr("transform", function () {
          if (_this8.model.state["activeTree"].includes(_this8.model.forest.rootNodeNames[treeIndex])) {
            return "translate(".concat(_this8._margin.left, ", ").concat(_this8._margin.top, ")");
          } else {
            return "translate(".concat(_this8._margin.left, ", ").concat(_this8.chartOffset, ")");
          }
        }).style("display", function () {
          if (_this8.model.state["activeTree"].includes("Show all trees")) {
            return "inline-block";
          } else if (_this8.model.state["activeTree"].includes(_this8.model.forest.rootNodeNames[treeIndex])) {
            return "inline-block";
          } else {
            return "none";
          }
        ...
        _this8.primary_legends[treeIndex].render();

        _this8.secondary_legends[treeIndex].render(); // Transition links to their new position.
        ...
        standardNodes.transition().duration(globals.duration).attr("transform", function (d) {
          return "translate(".concat(_this8._treeDepthScale(d.depth), ", ").concat(_this8._getLocalNodeX(d.x, treeIndex), ")");
        }); //update other characteristics of nodes

        standardNodes.select('circle.circleNode').style('stroke', function (d) {
          return 'black';
        }).style('stroke-width', function (d) {
          if (_this8.model.state['selectedNodes'].some(function (n) {
        ...
        links.exit().transition().duration(globals.duration).attr("d", function (d) {
          return _this8.diagonal(d.parent, d.parent, treeIndex);
        }).remove(); // make canvas always fit tree height

        if (_this8.model.state["activeTree"].includes("Show all trees") || _this8.model.state["activeTree"].includes(_this8.model.forest.rootNodeNames[treeIndex])) {
          _this8.chartOffset = _this8._treeLayoutHeights[treeIndex] + _this8.treeOffset + _this8._margin.top;
          _this8._height += _this8.chartOffset;
        }

        if (standardNodes.size() > nodeEnter.size()) {
          _this8.manageLabelCollisions(standardNodes);
        } else {
          _this8.manageLabelCollisions(nodeEnter);
        }
      };
...
function cct_tooltip_view_possibleConstructorReturn(self, call) { if (call && (cct_tooltip_view_typeof(call) === "object" || typeof call === "function")) { return call; } else if (call !== void 0) { throw new TypeError("Derived constructors may only return object or undefined"); } return cct_tooltip_view_assertThisInitialized(self); }

function cct_tooltip_view_assertThisInitialized(self) { if (self === void 0) { throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); } return self; }
...
var TooltipView = /*#__PURE__*/function (_View) {
  cct_tooltip_view_inherits(TooltipView, _View);

  var _super = cct_tooltip_view_createSuper(TooltipView);
   ...
   */
  function TooltipView(elem, model) {
    var _this;
  ...
  cct_tooltip_view_createClass(TooltipView, [{
    key: "render",
    value: function render() {
...
function cct_scented_slider_popup_possibleConstructorReturn(self, call) { if (call && (cct_scented_slider_popup_typeof(call) === "object" || typeof call === "function")) { return call; } else if (call !== void 0) { throw new TypeError("Derived constructors may only return object or undefined"); } return cct_scented_slider_popup_assertThisInitialized(self); }

function cct_scented_slider_popup_assertThisInitialized(self) { if (self === void 0) { throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); } return self; }
...
var ScentedSliderPopup = /*#__PURE__*/function (_View) {
  ...
  var _super = cct_scented_slider_popup_createSuper(ScentedSliderPopup);

  function ScentedSliderPopup(elem, model) {
    var _this;

    cct_scented_slider_popup_classCallCheck(this, ScentedSliderPopup);
    ...
    _this.full_hist_height = _this.popup_dims.height * .5;
    _this.num_bins = 25;
    _this.slider_width = 10;
    _this.slider_height = 20;
    _this.dropdown_clicked = false;
    _this._svg = build_d3.select(elem).append('svg').attr('width', _this.popup_dims['width']).attr('height', _this.popup_dims['height']).style('overflow', 'visible').style('position', 'absolute') // .style('left', (this.elem.clientWidth*.5 - this.popup_dims.width*.5) + 'px')
    .style('left', '145px').style('top', '150px').style('visibility', 'hidden');
    ...
    if (_this.max_zero_cnt > 0) {
      _this.hist_height = _this.full_hist_height * .7;
      _this.icycle_height = _this.full_hist_height - _this.hist_height;
    } else {
      _this.hist_height = _this.full_hist_height;
      _this.icycle_height = 0;
    }

    _this.slider_range = model.forest.forestMetrics[model.state.primaryMetric];
    _this.h_x_scale = build_d3.scaleLinear().domain([0, _this.bins.length]).range([0, _this.popup_dims['width'] - _this.popup_dims.left_padding - _this.popup_dims.right_padding]);
    _this.h_y_scale = build_d3.scaleLinear().domain(_this.bin_count_range).range([4, _this.hist_height]);
    _this.invert_y_scale = build_d3.scaleLinear().domain(_this.bin_count_range).range([_this.hist_height, 0]);
    _this.i_y_scale = build_d3.scaleLinear().domain([0, _this.max_zero_cnt]).range([0, _this.icycle_height]); //for communicating that sliding is over
  ...
  cct_scented_slider_popup_createClass(ScentedSliderPopup, [{
    key: "update_bin_count_ranges",
    value: function update_bin_count_ranges() {
      var _this2 = this;
    ...
    key: "manage_slider_update",
    value: function manage_slider_update(slider, x_pos) {
      /**
       * Callback function which manages updating the data and state associated with
       * the histogram sliders.
       */
      var self = this;
      var bin_num = parseInt(self.h_x_scale.invert(x_pos + this.slider_width / 2));
      var range_val = 0;
      var step_loc = 0;
      var update = false;
      ...
      if (update) {
        self.update = true;
        slider.attr('transform', "translate(".concat(step_loc - self.slider_width / 2, ",0)"));
        slider.select('text').text("".concat(getSigFigString(range_val)));
        slider.select('text').attr('x', function () {
          return -(this.getBBox().width / 2) + self.slider_width / 2;
        });
      }
    }
  }, {
    key: "handle_range_drag",
    value: function handle_range_drag(sld_node) {
      /**
       * Routing function which enables the correct
       * slider update arguments to be called depending on the location of the sliders.
       */
      var slider = build_d3.select(sld_node);
      var self = this;
      slider.attr('cursor', 'grabbing');

      if (build_d3.event.x > self.l_slider_pos_origin && build_d3.event.x < self.r_slider_pos_origin) {
        self.manage_slider_update(slider, build_d3.event.x);
      } else if (build_d3.event.x < self.l_slider_pos_origin) {
        self.manage_slider_update(slider, self.l_slider_pos_origin);
      } else if (build_d3.event.x > self.r_slider_pos_origin) {
    ...
    value: function pre_render() {
      ...
      var self = this;
      var start_x = 0;
      var start_y = 0; //Define drag functionality for the whole popup

      var windowDragHandler = build_d3.drag().on("end", function () {
        build_d3.select(this).attr('cursor', 'grab');
      }).on("drag", function () {
        build_d3.select(this).attr('cursor', 'grabbing');

        self._svg.style('left', parseInt(self._svg.style('left').slice(0, -2)) + build_d3.event.x - start_x + 'px');

        self._svg.style('top', parseInt(self._svg.style('top').slice(0, -2)) + build_d3.event.y - start_y + 'px');
      }).on("start", function () {
        self.update = false;
        start_x = build_d3.event.x;
        start_y = build_d3.event.y;
      }); //render histogram

      this._svg.append('rect').attr('height', this.popup_dims['height']).attr('width', this.popup_dims['width']).attr('fill', 'rgba(255,255,255,1)').attr('stroke', 'rgba(0,0,0,1)').attr('stroke-width', 2);

      var topBar = this._svg.append('rect').attr('height', '2em').attr('width', this.popup_dims['width']).attr('fill', 'rgba(255,255,255,0)').attr('stroke', 'rgba(0,0,0,1)').attr('stroke-width', 1).attr('cursor', 'grab');
      ...
      this._svg.append('g').attr('class', 'ice-grp').attr('height', this.icycle_height).attr('width', this.popup_dims['width']).attr('transform', "translate(".concat(this.popup_dims.left_padding, ",").concat(this.popup_dims.top_padding + 5 + this.hist_height, ")"));

      this.hist_grp = this._svg.select('.hist-grp');
      this.ice_grp = this._svg.select('.ice-grp');
      this.hist_grp.append('g').attr('class', 'left-axis').attr('transform', "translate(0, 0)").call(build_d3.axisLeft(this.invert_y_scale).ticks(4));
      this.ice_grp.append('g').attr('class', 'left-ice-axis').call(build_d3.axisLeft(this.i_y_scale).ticks(4));
      ...
      var dragHandler = build_d3.drag().on("end", function () {
        if (self.update) {
          self.observers.notify({
      ...
      var l_slider = this.hist_grp.append('g').attr('class', 'l-slider-grp').attr('transform', "translate(".concat(this.l_slider_pos_origin, ",0)")).attr('cursor', 'grab');
      l_slider.append('rect').attr('class', 'left-handle').attr('width', this.slider_width).attr('height', this.slider_height).attr('y', this.full_hist_height + 2).attr('fill', 'rgba(200,150,150,1)').attr('stroke', 'rgba(0,0,0,1)');
      l_slider.append('line').attr('class', 'left-line').attr('x1', this.slider_width / 2).attr('x2', this.slider_width / 2).attr('y1', 0).attr('y2', this.full_hist_height + 2).attr('stroke', 'rgba(200,50,50,1)');
      var txt = l_slider.append('text').text("".concat(getSigFigString(this.bins[parseInt(this.h_x_scale.invert(this.l_slider_pos_origin))].x0))).attr('y', this.full_hist_height + 2 + this.slider_height + 14);
      txt.attr('x', function () {
        return -(this.getBBox().width / 2) + self.slider_width / 2;
      });
      var r_slider = this.hist_grp.append('g').attr('class', 'r-slider-grp').attr('transform', "translate(".concat(this.r_slider_pos_origin, ",0)")).attr('cursor', 'pointer');
      r_slider.append('rect').attr('class', 'right-handle').attr('width', this.slider_width).attr('height', this.slider_height).attr('y', this.full_hist_height + 2).attr('fill', 'rgba(200,150,150,1)').attr('stroke', 'rgba(0,0,0,1)');
      r_slider.append('line').attr('class', 'right-line').attr('x1', this.slider_width / 2).attr('x2', this.slider_width / 2).attr('y1', 0).attr('y2', this.full_hist_height + 2).attr('stroke', 'rgba(200,50,50,1)');
      txt = r_slider.append('text').text("".concat(getSigFigString(this.bins[parseInt(this.h_x_scale.invert(this.r_slider_pos_origin))].x1))).attr('y', this.full_hist_height + 2 + this.slider_height + 14);
      txt.attr('x', function () {
        return -(this.getBBox().width / 2) + self.slider_width / 2;
      });
      dragHandler(l_slider);
      dragHandler(r_slider); //Selection box area

      var select_line = this._svg.append('g').attr('class', 'distribution_selection').attr('transform', "translate(10, ".concat(this.full_hist_height + this.slider_height + topBar.node().getBBox().height + 40, ")"));

      var info_txt = select_line.append('text').text('Current metric distribution: ');
      var select_box = select_line.append('g').attr('class', 'selection-button').attr('cursor', 'default').on('mouseenter', function () {
        select_area.attr('fill', 'rgba(100,100,200,1)');
        self.current_selection_text.attr('fill', 'rgba(255,255,255,.9)');
      }).on('mouseleave', function () {
        if (!self.dropdown_clicked) {
          select_area.attr('fill', 'rgba(255,255,255,1)');
      ...
      var select_area = select_box.append('rect').attr('y', function () {
        return -info_txt.node().getBBox().height;
      }).attr('x', info_txt.node().getBBox().width + 10).attr('fill', 'rgba(255,255,255,1)').attr('stroke', 'rgba(0,0,0,1)').attr('stroke-width', 1);
      this.current_selection_text = select_box.append('text').text(function () {
        return _this3.model.state.primaryMetric;
      }).attr('x', function () {
        return info_txt.node().getBBox().width + 12;
      });
      this.options_dropdown = select_box.append('g').style('visibility', 'hidden');
      select_area.attr('height', function () {
        return _this3.current_selection_text.node().getBBox().height + 4;
      }).attr('width', function () {
        return _this3.current_selection_text.node().getBBox().width + 10;
      });
    }
  }, {
    key: "render",
    value: function render() {
      ...
      var self = this;
      var options = this.model.forest.metricColumns;
      ...
      var met_options = this.options_dropdown.selectAll('.metric-option').data(options);
      /**
       * 
       * ENTER
       * 
      ...
      var sel_btn_bbx = this._svg.select('.selection-button').node().getBBox();

      var max_option_width = 0;
      var max_option_height = 0;
      var option_button = met_options.enter().append('g').attr('class', 'metric-option').attr('transform', function (_, i) {
        return "translate(".concat(sel_btn_bbx.x, ", ").concat(i * sel_btn_bbx.height + sel_btn_bbx.height / 4, ")");
      }).on('click', function (d) {
        self.observers.notify({
          type: globals.signals.METRICCHANGE,
          newMetric: d,
        ...
        build_d3.select(this).select('rect').attr('fill', 'rgba(100,100,200,1)');
        build_d3.select(this).select('text').attr('fill', 'rgba(255,255,255,.9)');
      }).on('mouseleave', function () {
        build_d3.select(this).select('rect').attr('fill', 'rgba(255,255,255,1)');
        build_d3.select(this).select('text').attr('fill', 'rgba(0,0,0,.9)');
      });
      var option_rects = option_button.append('rect').attr('height', sel_btn_bbx.height).attr('fill', 'rgba(255,255,255,1)').attr('stroke', 'rgba(0,0,0,1)').attr('stroke-width', 1);
      var opt_txt = option_button.append('text').text(function (d) {
        return d;
      }).attr('x', 2);
      opt_txt.each(function () {
        ...
        build_d3.select(this).attr('y', sel_btn_bbx.height / 2 + this.getBBox().height / 2);
      });
      bars.enter().append('rect').attr('class', 'hist-bar').attr('width', (this.popup_dims['width'] - this.popup_dims.left_padding - this.popup_dims.right_padding) / this.bins.length).attr('height', function (d) {
        if (d.length != 0) {
          return _this4.h_y_scale(d.length);
        }

        return 0;
      }).attr('x', function (_, i) {
        return _this4.h_x_scale(i);
      }).attr('y', function (d) {
        if (d.length != 0) {
          return _this4.hist_height - _this4.h_y_scale(d.length); //- v_bar_negative_padding;
        }

        return _this4.hist_height;
      }).attr('fill', function (_, i) {
        if (i >= _this4.current_l_bin && i <= _this4.current_r_bin) {
          return 'rgba(100,100,200,1)';
        }
      }).attr('stroke', 'rgba(0,0,0,1)').attr('stroke-width', '1px').on('mouseenter', function (d) {
        _this4.observers.notify({
      ...
      rev_bars.enter().append('rect').attr('class', 'ice-bar').attr('width', (this.popup_dims['width'] - this.popup_dims.left_padding - this.popup_dims.right_padding) / this.bins.length).attr('height', function (d) {
        if (d.length != 0) {
          return _this4.i_y_scale(d.length);
        }

        return 0;
      }).attr('x', function (_, i) {
        return _this4.h_x_scale(i);
      }).attr('y', 0).attr('fill', function (_, i) {
      ...
      bars.attr('fill', function (_, i) {
        ...
        return 'rgba(200,200,200,1';
      }).transition().duration(750).attr('width', (this.popup_dims['width'] - this.popup_dims.left_padding - this.popup_dims.right_padding) / this.bins.length).attr('height', function (d) {
        if (d.length != 0) {
          return _this4.h_y_scale(d.length);
        }
      ...
      rev_bars.transition().duration(750).attr('width', (this.popup_dims['width'] - this.popup_dims.left_padding - this.popup_dims.right_padding) / this.bins.length).attr('height', function (d) {
        if (d.length != 0) {
          return _this4.i_y_scale(d.length);
        }
  ...
  return ScentedSliderPopup;
}(view);

/* harmony default export */ const cct_scented_slider_popup = (ScentedSliderPopup);
;// CONCATENATED MODULE: ./scripts/cct.js
//d3.v4
...
build_d3.select(element).attr('width', '100%');
RT.jsNodeSelected = JSON.stringify(["*"]); // ---------------------------------------------
// Main driver area 
// ---------------------------------------------
//model

var model = new cct_model(); //controller

var cont = new cct_controller(model); //views

var menu = new cct_menu_view(element, model);
var tooltip = new cct_tooltip_view(element, model);
var chart = new cct_chart_view(element, model);
var popup = new cct_scented_slider_popup(element, model); //register signallers 
...
model.register(chart.renderer()); //render all views one time
