...
class Modeling:
    ...
    def __init__(self, gfs, params, param_name, all_met=None, depth=10000):
        ...
        # Unify all graphs -- this is needed so that we have the same callgraph across all
        # the frames
    ...
    # Produces an Extra-P model by creating all the objects that Extra-P usually creates.
    # It means all the objects that represent values, metrics, and data points. Models are
    # generated by calling Extra-P's ModelGenerator.
    ...
    # Traverses the call graphs in sync. For each set of nodes at the same locations
    # across the graphs - produce a moodel by calling _produce_extrap_models. This
    # function has the same structure as other GraphFrame traversal functions in Hatchet.
