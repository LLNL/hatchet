...
def test_multi_rank(caliper_multi_rank):
    ...
    node = [n for n in gf.dataframe.index.get_level_values("node") if n.frame["name"] == "CalculateMoments"][0]
    ...
    assert len(gf.dataframe.index.get_level_values("rank").unique()) == gf.metadata["mpi.world.size"]