...
class CaliperNativeReader:
    ...
    __cali_type_dict = {
        "inv": lambda dummy: __raise_cali_type_error("Caliper type 'inv' is unsupported in Hatchet"),
        "usr": lambda dummy: __raise_cali_type_error("Custom Caliper types are unsupported in Hatchet"),
        "int": int,
        "uint": int,
        "string": str,
        "addr": int,
        "double": float,
        "bool": bool,
        "type": lambda dummy: __raise_cali_type_error("Caliper 'type' types are unsupported in Hatchet"),
        "ptr": lambda dummy: __raise_cali_type_error("Caliper 'ptr' types are for internal use only!"),
    }
                    ...
                    for item in record.keys():
                        if item != ctx:
                            ...
                            if attr_type in self.__cali_type_dict:
                                if attr_type != "string" or item in self.string_attributes:
                                    try:
                                        node_dict[item] = self.__cali_type_dict[attr_type](record[item])
